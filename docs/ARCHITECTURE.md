...
Issue #45 で `apps/app` に `slang`（多言語化）のコード生成が必要になったが、対象は
`apps/app` 単体でありパッケージを横断しないため、これは上記の再検討基準に該当しない。
`build_runner` 2.14.0 で正式化された `--workspace` フラグにより、Melos を導入せずに
リポジトリルートからコード生成できる（`docs/development.md` 参照）。複数パッケージが
コード生成の対象になった時点で、改めて Melos 導入の要否を検討する。
