...
| `packages/designsystem` | テーマ、共通 Widget、画面に依存しない UI 表現。デザイン方針（MD3準拠・レスポンシブ対応の判断基準・ブレークポイント等）は `docs/design.md` を参照 |
...
リポジトリルートからコード生成できるようにしている（`docs/development.md` 参照）。
複数パッケージがコード生成の対象になった時点で、改めて Melos 導入の要否を検討する。
